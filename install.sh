...
printf "
#######################################################################
#       ServerSupport for CentOS/RadHat 5+ Debian 6+ and Ubuntu 12+   #
#                                                                     #
#######################################################################
"

# get pwd
sed -i "s@^asn_dir.*@asn_dir=`pwd`@" ./options.conf
...
case "${OS}" in
  "CentOS")
    installDepsCentOS 2>&1 | tee ${asn_dir}/install.log
    ;;
  "Debian")
    installDepsDebian 2>&1 | tee ${asn_dir}/install.log
    ;;
  "Ubuntu")
    installDepsUbuntu 2>&1 | tee ${asn_dir}/install.log
    ;;
esac
...
case "${OS}" in
  "CentOS")
    . include/init_CentOS.sh 2>&1 | tee -a ${asn_dir}/install.log
    [ -n "$(gcc --version | head -n1 | grep '4\.1\.')" ] && export CC="gcc44" CXX="g++44"
    ;;
  "Debian")
    . include/init_Debian.sh 2>&1 | tee -a ${asn_dir}/install.log
    ;;
  "Ubuntu")
    . include/init_Ubuntu.sh 2>&1 | tee -a ${asn_dir}/install.log
    ;;
esac
...
downloadDepsSrc=1
checkDownload 2>&1 | tee -a ${asn_dir}/install.log

# Install dependencies from source package
installDepsBySrc 2>&1 | tee -a ${asn_dir}/install.log

# Jemalloc
if [[ $Nginx_version =~ ^[1-3]$ ]] || [ "$DB_yn" == 'y' -a "$DB_version" != '10' ]; then
  . include/jemalloc.sh
  Install_Jemalloc | tee -a $asn_dir/install.log
fi
...
case "${DB_version}" in
  1)
    if [ "${dbInstallMethods}" == "2" ]; then
      . include/boost.sh
      installBoost 2>&1 | tee -a ${asn_dir}/install.log
    fi
    . include/mysql-5.7.sh
    Install_MySQL57 2>&1 | tee -a ${asn_dir}/install.log
    ;;
  2)
    . include/mysql-5.6.sh
    Install_MySQL56 2>&1 | tee -a ${asn_dir}/install.log
    ;;
  3)
    . include/mysql-5.5.sh
    Install_MySQL55 2>&1 | tee -a ${asn_dir}/install.log
    ;;
  4)
    if [ "${dbInstallMethods}" == "2" ]; then
      . include/boost.sh
    ...
    Install_MariaDB101 2>&1 | tee -a ${asn_dir}/install.log
    ;;
  5)
    . include/mariadb-10.0.sh
    Install_MariaDB100 2>&1 | tee -a ${asn_dir}/install.log
    ;;
  6)
    . include/mariadb-5.5.sh
    Install_MariaDB55 2>&1 | tee -a ${asn_dir}/install.log
    ;;
  7)
    if [ "${dbInstallMethods}" == "2" ]; then
      . include/boost.sh
    ...
    Install_Percona57 2>&1 | tee -a ${asn_dir}/install.log
    ;;
  8)
    . include/percona-5.6.sh
    Install_Percona56 2>&1 | tee -a ${asn_dir}/install.log
    ;;
  9)
    . include/percona-5.5.sh
    Install_Percona55 2>&1 | tee -a ${asn_dir}/install.log
    ;;
  10)
    . include/alisql-5.6.sh
    Install_AliSQL56 2>&1 | tee -a $asn_dir/install.log
    ;;
esac
...
if [ "$Apache_version" == '1' ]; then
  ...
  Install_Apache24 2>&1 | tee -a $asn_dir/install.log
elif [ "$Apache_version" == '2' ]; then
  . include/apache-2.2.sh
  Install_Apache22 2>&1 | tee -a $asn_dir/install.log
fi
...
case "${PHP_version}" in
  1)
    . include/php-5.3.sh
    Install_PHP53 2>&1 | tee -a ${asn_dir}/install.log
    ;;
  2)
    . include/php-5.4.sh
    Install_PHP54 2>&1 | tee -a ${asn_dir}/install.log
    ;;
  3)
    . include/php-5.5.sh
    Install_PHP55 2>&1 | tee -a ${asn_dir}/install.log
    ;;
  4)
    . include/php-5.6.sh
    Install_PHP56 2>&1 | tee -a ${asn_dir}/install.log
    ;;
  5)
    . include/php-7.0.sh
    Install_PHP70 2>&1 | tee -a ${asn_dir}/install.log
    ;;
  6)
    . include/php-7.1.sh
    Install_PHP71 2>&1 | tee -a ${asn_dir}/install.log
    ;;
esac
...
if [ "$Magick" == '1' ]; then
  ...
  [ ! -d "/usr/local/imagemagick" ] && Install_ImageMagick 2>&1 | tee -a $asn_dir/install.log
  [ ! -e "`$php_install_dir/bin/php-config --extension-dir`/imagick.so" ] && Install_php-imagick 2>&1 | tee -a $asn_dir/install.log
elif [ "$Magick" == '2' ]; then
  . include/GraphicsMagick.sh
  [ ! -d "/usr/local/graphicsmagick" ] && Install_GraphicsMagick 2>&1 | tee -a $asn_dir/install.log
  [ ! -e "`$php_install_dir/bin/php-config --extension-dir`/gmagick.so" ] && Install_php-gmagick 2>&1 | tee -a $asn_dir/install.log
fi

# ionCube
if [ "$ionCube_yn" == 'y' ]; then
  . include/ioncube.sh
  Install_ionCube 2>&1 | tee -a $asn_dir/install.log
fi
...
case "${PHP_cache}" in
  1)
    if [[ "${PHP_version}" =~ ^[1,2]$ ]]; then
      ...
      Install_ZendOPcache 2>&1 | tee -a ${asn_dir}/install.log
    fi
    ;;
  2)
    . include/xcache.sh
    Install_XCache 2>&1 | tee -a ${asn_dir}/install.log
    ;;
  3)
    . include/apcu.sh
    Install_APCU 2>&1 | tee -a ${asn_dir}/install.log
    ;;
  4)
    if [[ "${PHP_version}" =~ ^[1,2]$ ]]; then
      . include/eaccelerator.sh
      Install_eAccelerator 2>&1 | tee -a ${asn_dir}/install.log
    fi
    ;;
esac
...
if [ "$ZendGuardLoader_yn" == 'y' ]; then
  ...
  Install_ZendGuardLoader 2>&1 | tee -a $asn_dir/install.log
fi
...
case "${Nginx_version}" in
  1)
    . include/nginx.sh
    Install_Nginx 2>&1 | tee -a ${asn_dir}/install.log
    ;;
  3)
    . include/openresty.sh
    Install_OpenResty 2>&1 | tee -a ${asn_dir}/install.log
    ;;
esac
...
if [ "${FTP_yn}" == 'y' ]; then
  . include/pureftpd.sh
  Install_PureFTPd 2>&1 | tee -a ${asn_dir}/install.log
fi

# phpMyAdmin
if [ "${phpMyAdmin_yn}" == 'y' ]; then
  . include/phpmyadmin.sh
  Install_phpMyAdmin 2>&1 | tee -a ${asn_dir}/install.log
fi

# memcached
if [ "${memcached_yn}" == 'y' ]; then
  . include/memcached.sh
  [ ! -d "${memcached_install_dir}/include/memcached" ] && Install_memcached 2>&1 | tee -a ${asn_dir}/install.log
  [ -e "${php_install_dir}/bin/phpize" ] && [ ! -e "$(${php_install_dir}/bin/php-config --extension-dir)/memcache.so" ] && Install_php-memcache 2>&1 | tee -a ${asn_dir}/install.log
  [ -e "${php_install_dir}/bin/phpize" ] && [ ! -e "$(${php_install_dir}/bin/php-config --extension-dir)/memcached.so" ] && Install_php-memcached 2>&1 | tee -a ${asn_dir}/install.log
fi

# index example
if [ ! -e "${wwwroot_dir}/default/index.html" -a "${Web_yn}" == 'y' ]; then
  . include/demo.sh
  DEMO 2>&1 | tee -a ${asn_dir}/install.log
fi
...
echo "Total AsnTechSolution Install Time: ${CQUESTION}${installTime}${CEND} minutes"
